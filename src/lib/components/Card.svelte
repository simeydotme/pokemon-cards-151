<script>
  ...
  const staticStyles = `
    --seedx: ${randomSeed.x};
    --seedy: ${randomSeed.y};
    --cosmosbg: ${bgPosition1.x}px ${bgPosition1.y}px;
    --birthdaybg: ${bgPosition2.x}px ${bgPosition2.y}px;
  `;
  $: dynamicStyles = `
    ${staticStyles}
    ${foilStyles}
    ...
    --background-x: ${clamp($springBackground.x)}%;
    --background-y: ${clamp($springBackground.y)}%;
